...
function RunProg([string]$strategy, [string]$param) {
	Get-ChildItem -Path ".\puzzles" -File | Where-Object { $_.Name -match $InitFilenameRegex } | ForEach-Object {
        ...
        $SolFilename = '.\results\{0}_sol.txt' -f $FilenameRoot
        $StatsFilename = '.\stats\{0}_stats.txt' -f $FilenameRoot
        Invoke-Expression $('{0} {1} {2} {3} {4} {5}' -f $Progcmd, $strategy, $param, $_.FullName, $SolFilename,
                            $StatsFilename)
    }
}
